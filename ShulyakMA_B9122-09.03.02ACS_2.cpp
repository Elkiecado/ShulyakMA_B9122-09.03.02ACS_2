...
int main()
{
	...
	std::cout << "Количество используемых потоков: " << threads << "\n";
...
#pragma omp for reduction(+:s1)
	...
	t2 = clock();
	std::cout << "Время затраченное на суммирование float: " << t2 - t1 << "\n";
	t3 = clock();
#pragma omp parallel
	{
#pragma omp for reduction(+:s2)
	...
	t4 = clock();
	std::cout << "Время затраченное на суммирование double: " << t4 - t3 << "\n";
	const int n2 = 2;
...
#pragma omp for reduction(+:s1)
	...
	t2 = clock();
	std::cout << "Время затраченное на произведение float: " << t2 - t1 << "\n\n";
	s2 = 0;
	t3 = clock();
#pragma omp parallel
	{
#pragma omp for reduction(+:s2)
	...
	t4 = clock();
	std::cout << "Время затраченное на произведение double: " << t4 - t3 << "\n\n";
}